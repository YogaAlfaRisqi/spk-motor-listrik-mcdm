...
<style>
    ...
    .motor-card.is-selected .btn-compare { background:#C8F135; color:#000; border-color:#C8F135; }
    ...
    .pg-btn {
        ...
        background: #14161A; color: rgba(255,255,255,.5);
        cursor: pointer; transition: all .15s;
        padding: 0 8px;
    }
    .pg-btn:hover { border-color: rgba(200,241,53,.4); color: #C8F135; }
    .pg-btn.active { background:#C8F135; color:#000; border-color:#C8F135; }
    .pg-btn:disabled { opacity:.3; cursor:default; pointer-events:none; }
    .pg-dots { color:rgba(255,255,255,.25); font-size:12px; padding:0 4px; }
    ...
    .bar-chip {
        ...
        min-width: 152px; max-width: 180px;
        position: relative;
    }
    .bar-chip img { width:38px;height:38px;border-radius:8px;object-fit:cover;flex-shrink:0; }
    .bar-chip-info { flex:1;min-width:0; }
    .bar-chip-name { font-size:10px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#fff; }
    ...
    @media (max-width: 639px) {
        ...
        .card-price-txt { font-size: 12px !important; margin-bottom: 10px !important; }
        .spec-box { padding: 6px 8px !important; }
        .spec-label-txt { font-size: 7px !important; }
        .spec-val-txt { font-size: 10px !important; }
...
(function () {
    ...
    function renderGrid() {
        ...
        grid.innerHTML = slice.map(m => {
            ...
            <div
                ...
                data-id="${m.id}" data-name="${m.name}" data-price="${m.price}" data-img="${m.img}"
            >
                ...
                <div class="card-body-pad p-4 sm:p-5 flex flex-col flex-1">
                    ...
                    <div class="grid grid-cols-2 gap-1.5 mb-4">
                        <div class="spec-box bg-[#0A0C0F] p-2.5 rounded-xl border border-white/5">
                            ...
                            <p class="spec-val-txt text-[10px] sm:text-xs font-bold">${m.range}</p>
                        </div>
                        <div class="spec-box bg-[#0A0C0F] p-2.5 rounded-xl border border-white/5">
                            <p class="spec-label-txt text-[7px] sm:text-[8px] text-gray-500 uppercase font-bold mb-0.5">Top Speed</p>
                            <p class="spec-val-txt text-[10px] sm:text-xs font-bold">${m.speed}</p>
                        </div>
                    </div>
                    <div class="mt-auto">
                        <button
    ...
    window.prosesPerbandingan = function () {
        if (selected.length < 2) { alert('Pilih minimal 2 motor untuk dibandingkan.'); return; }
    ...
    function renderBar() {
        ...
        barItems.innerHTML = selected.map(id => {
            const m = ALL_MOTORS.find(x => x.id === id);
            if (!m) return '';
            return `
                <div class="bar-chip">
                    <img src="${m.img}" alt="${m.name}">
    ...
    barItems.addEventListener('mousedown', e => { drag=true; sx=e.pageX; ss=barItems.scrollLeft; barItems.style.cursor='grabbing'; e.preventDefault(); });
    document.addEventListener('mousemove', e => { if(drag) barItems.scrollLeft = ss-(e.pageX-sx); });
    document.addEventListener('mouseup',   () => { drag=false; barItems.style.cursor='grab'; });
