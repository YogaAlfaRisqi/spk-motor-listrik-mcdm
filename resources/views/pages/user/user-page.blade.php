...
<div class="space-y-6">
    ...
    <div class="p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
        ...
        <p class="text-gray-500 dark:text-gray-400 mb-6">
            Kelola pengguna sistem, atur peran, dan kontrol akses dengan mudah.
        </p>
...
<div class="space-y-6 py-10">
    ...
    dropdownOpen: null,
    get totalPages() {
        return Math.ceil(this.criteria.length / this.itemsPerPage);
    },
    get paginatedCriteria() {
        const start = (this.currentPage - 1) * this.itemsPerPage;
        const end = start + this.itemsPerPage;
        return this.criteria.slice(start, end);
    },
    get displayedPages() {
        const range = [];
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="border-gray-200 border-y dark:border-gray-700">
                                ...
                                <th scope="col" class="px-4 py-3 text-xs font-medium tracking-wider text-left text-gray-500 capitalize">Satuan</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <template x-for="criteria in paginatedCriteria" :key="criteria.id">
                                <tr>
                                    <td class="py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <!-- number content -->
                                             <div>
                                                <div class="text-sm font-medium text-gray-900 dark:text-white" x-text="criteria.id"></div>
                                            </div>

                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900 dark:text-white" x-text="criteria.name"></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 dark:text-gray-400" x-text="criteria.date"></div>
                                    </td>

                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500 dark:text-gray-400" x-text="criteria.category"></div>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="getStatusClass(criteria.status)" x-text="criteria.status"></span>
                                    </td>
